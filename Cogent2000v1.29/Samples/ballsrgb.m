function ballsrgb(Size,Speed,GMode)
% Sample script ballsrgb v1.29
%
% Creates a display of coloured circles bouncing around
% at different speeds.  The display is in direct colour mode.
%
% Usage:-   ballsrgb(Size,Speed,GMode)
%
...
for i = 1:7
   ...
   cgmakesprite(i,2*Size*j,2*Size*j,0,0,0)
   cgtrncol(i,'n')
   cgsetsprite(i)
%
% Convert 'j' into a colour.
%
% This next bit of code sets the following colours
%
% i = 1  j = 7  r,g,b = 1,1,1 = white 
% i = 2  j = 6  r,g,b = 0,1,1 = cyan 
% i = 3  j = 5  r,g,b = 1,0,1 = magenta 
% i = 4  j = 4  r,g,b = 0,0,1 = blue 
% i = 5  j = 3  r,g,b = 1,1,0 = yellow 
% i = 6  j = 2  r,g,b = 0,1,0 = green 
% i = 7  j = 1  r,g,b = 1,0,0 = red 
%
   r = j - 2*round((j - 1)/2);
   g = round((j - 1)/2) - 2*round((j - 2)/4);
   b = round((j - 2)/4);
...
%   
   cgalign('c','t')
   str = sprintf('ballsrgb v1.29 P:%s Tim:%02d:%02d:%02d Frm:%d Av:%-.2fHz Drp:%d',...
   cogstd('spriority'),...
