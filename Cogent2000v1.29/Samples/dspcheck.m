...
if XYZFlag == 1
    ...
    for i=1:ChkDat.NumVals
        k = i;
        ChkDat.RGB(i,1) = ChkDat.Lev(1 + mod(k,Levels));
        k = fix(k/Levels);
        ChkDat.RGB(i,2) = ChkDat.Lev(1 + mod(k,Levels));
        k = fix(k/Levels);
        ChkDat.RGB(i,3) = ChkDat.Lev(1 + mod(k,Levels));
        ChkDat.XYZIn(i,:) = RGB2XYZ(ChkDat.RGB(i,:));
    end
end
%
...
if (cgphotometer('Open',PhotometerID,PortNum) ~= 0)
    ...
    %
        ...
        for i=1:NumVals
			...
			%
            ChkDat.XYZOut(i,:) = cgphotometer('XYZ');
        end
	    %
		% Close the graphics
	    %
...
fprintf('    PhotometerID = Which photometer to use (eg. ''PR650'')\n')
