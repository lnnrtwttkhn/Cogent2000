...
if tm.FrmCnt > 0
    str = sprintf('tennispal v1.29 P:%s Tim:%02d:%02d:%02d Frm:%d Av:%-.2fHz Drp:%d',...
        cogstd('spriority'),...
	...
	str = sprintf('tennispal v1.29 P:%s Tim:%02d:%02d:%02d Frm:%d Drp:%d',...
        cogstd('spriority'),...
        fix(S/3600),mod(fix(S/60),60),mod(fix(S),60),...
        tm.FrmCnt,...
        tm.Drp);
...
fprintf('\nusage - tennispal(BatLength,BallSize)\n\n')
